...
export class CorpusProcessor {
  ...
  async processSuccessPatterns(): Promise<SuccessPattern[]> {
  ...
  async processLanguagePatterns(): Promise<LanguagePattern[]> {
  ...
  async saveProcessedData(
    ...
    writeFileSync(
      join(dataDir, 'success-patterns.json'),
      JSON.stringify(successPatterns, null, 2)
    )
    
    writeFileSync(
      join(dataDir, 'language-patterns.json'),
      JSON.stringify(languagePatterns, null, 2)
    )
