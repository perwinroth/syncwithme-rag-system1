...
import { TravelIntent, LanguagePattern, RAGResult } from '../types'
...
export class IntentExtractor {
  ...
  async extractIntent(userMessage: string): Promise<TravelIntent> {
  ...
  private async extractWithAI(userMessage: string): Promise<Partial<TravelIntent>> {
    const prompt = `
...
`
  ...
  private extractFromPatterns(
    userMessage: string, 
    patterns: RAGResult[]
  ): Partial<TravelIntent> {
    const message = userMessage.toLowerCase()
    let confidence = 0
    const extracted: any = {}

    for (const pattern of patterns) {
      if (pattern.type !== 'language_pattern') continue
      
      const langPattern = pattern.content as LanguagePattern
      
      // Check if any phrases match
      const matchingPhrases = langPattern.phrases.filter(phrase => 
  ...
  private mergeIntents(
    aiIntent: Partial<TravelIntent>,
    patternIntent: Partial<TravelIntent>, 
    originalText: string
  ): TravelIntent {
    ...
    const merged = {
      ...
      originalText,
      confidence: Math.max(
        aiIntent.confidence || 0,
        patternIntent.confidence || 0,
        0.5 // Minimum baseline
      )
    }

    return merged as TravelIntent
  }
  ...
  async learnFromInteraction(
    ...
    const newPatterns: LanguagePattern[] = []
  ...
  private extractBudgetPhrases(message: string): string[] {
